...
import changeSpotVisibility from "@/firebase/changeSpotVisibility";
...
export default function List({
  ...
  const handleChangeSpotVisibility = async (id, value) => {
    try {
      await changeSpotVisibility(id, value);
      alert(`已${value ? "隱藏" : "顯示"}該景點。`);
    } catch (error) {
      console.error("景點隱藏狀態更新失敗:", error);
      alert("景點隱藏狀態更新失敗，請重試。");
    }
  };
            ...
            <div className="flex flex-row justify-around justify-self-stretch">
              ...
              {spot.hidden ? (
                <svg
                  ...
                  className="h-6 w-6 cursor-pointer text-gray-600 hover:text-red-500"
                  onClick={() => {
                    handleChangeSpotVisibility(spot.id, !spot.hidden);
                  }}
                >
                  <path
                    strokeLinecap="round"
